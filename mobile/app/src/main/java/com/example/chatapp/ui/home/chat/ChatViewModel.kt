...
class ChatViewModel(application: Application) : AndroidViewModel(application) {
                        ...
                        val existingMessage = currentMessages.find { message ->
                            ...
                            if (messageId.isNotEmpty() && message.id?.toString() == messageId) {
                                ...
                                message.content == messageContent &&
                                        ...
                                        Math.abs(parseTimestamp(message.timestamp) - parseTimestamp(messageTimestamp)) < 3000 // 3 giây tolerance
                            }
                        }
     ...
     */
    fun syncMessagesWhenReconnected(conversationId: String) {
        viewModelScope.launch {
            try {
                        ...
                        val newMessages = serverMessages.filter { serverMessage ->
                            currentMessages.none { localMessage ->
                                localMessage.content == serverMessage.content &&
                                        ...
                                        Math.abs(parseTimestamp(localMessage.timestamp) - parseTimestamp(serverMessage.timestamp)) < 5000
                            }
                        }
